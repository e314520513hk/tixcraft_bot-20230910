...
def get_default_config():
    ...
    config_dict["advanced"]["row_to_search"] = 0
    return config_dict
...
def btn_save_act(language_code, slience_mode=False):
    ...
    if is_all_data_correct:
        ...
        config_dict["advanced"]["row_to_search"] = int(txt_row_to_search.get().strip())
...
def AdvancedTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    ##年代專用 要搜尋的排數
    global lbl_row_to_search
    lbl_row_to_search = Label(frame_group_header, text=translate[language_code]['row_to_search'])
    lbl_row_to_search.grid(column=0, row=group_row_count, sticky = E)

    global txt_row_to_search
    txt_row_to_search_value = StringVar(frame_group_header, value=config_dict["advanced"]["row_to_search"])
    txt_row_to_search = Entry(frame_group_header, width=30, textvariable = txt_row_to_search_value)
    txt_row_to_search.grid(column=1, row=group_row_count, sticky = W)
