...
def kham_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    if '.aspx?performance_id=' in url.lower() and 'performance_price_area_id=' in url.lower():
        ...
        try:
            current_url, is_quit_bot = get_current_url(driver)
            if config_dict["aa_current_url"] != current_url:
                config_dict["ticket_selected"] = False
            if not config_dict["ticket_selected"]:
                ...
                try:
                    ...
                    bb_btn =driver.find_element(By.CSS_SELECTOR, "#portfolio > div.container > div:nth-child(2) > div > div > div.row > div:nth-child(4) > div > div > div > ul > li > a")
                    if not bb_btn is None:
                        driver.execute_script("document.querySelector('#portfolio > div.container > div:nth-child(2) > div > div > div.row > div:nth-child(4) > div > div > div > ul > li > a').click();")        
                except Exception as exc:
                    pass
                try:
                    
                    aa_btn =driver.find_element(By.CSS_SELECTOR, "#portfolio > div.container > div:nth-child(2) > div > div > div.row > div:nth-child(4) > div > div > div > ul > li:nth-child(2) > a")
                    if not aa_btn is None:
                        driver.execute_script("document.querySelector('#portfolio > div.container > div:nth-child(2) > div > div > div.row > div:nth-child(4) > div > div > div > ul > li:nth-child(2) > a').click();")        
                except Exception as exc:
                    pass

                try:
                    
                    driver.find_element(By.CSS_SELECTOR, "#content > div.flex.chooseSeat > div.chooseSeatMAP > div:nth-child(1) > div.ticket > button.green")
                    driver.execute_script("document.querySelector('#content > div.flex.chooseSeat > div.chooseSeatMAP > div:nth-child(1) > div.ticket > button.green').click();")      
                except Exception as exc:
                    pass

                
                row_to_search = str(config_dict["advanced"]["row_to_search"])
                # add_to_cart_btn =driver.find_element(By.CSS_SELECTOR, "#ctl00_ContentPlaceHolder1_LOGINED_BLOCK > button")
                
                if is_captcha_sent:
                    # if not aa_btn is None or not bb_btn is None:
                    time.sleep(0.5)
                    # driver.execute_script("const empty_chairs = [];document.querySelector('#TBL > tbody').childNodes["+row_to_search+"].childNodes.forEach((e)=>{if(e.style.backgroundImage.indexOf('empty')>0)empty_chairs.push(e)});empty_chairs[Math.round((empty_chairs.length-1)/2)].click();empty_chairs[Math.round((empty_chairs.length-1)/2)+1].click();")        
                    driver.execute_script("const empty_chairs=[];document.querySelector('#TBL > tbody').childNodes.forEach((e)=>{e.childNodes.forEach((e)=>{if(e.getAttribute('class')){if(e.getAttribute('class').indexOf('empty')==0)empty_chairs.push(e)}})});empty_chairs[0].click();empty_chairs[1].click();")        
                    try:
                    
                        driver.execute_script("document.querySelector('#ctl00_ContentPlaceHolder1_LOGINED_BLOCK > button').click();")
                        config_dict["ticket_selected"] = True
                    except Exception as exc:
                        pass
                    try:
                    
                        driver.execute_script("document.querySelector('#addcart > button').click();")
                        config_dict["ticket_selected"] = True
                    except Exception as exc:
                        pass
    ...
    config_dict["aa_current_url"], is_quit_bot = get_current_url(driver) 
